Write-Host "task_SO WHS SEND SALES.ps1 - Version 1.0"
# ScriptVersion-1.0
# Checks for and creates a scheduled task to run SOScheduler.exe with the WHSSENDSALES D argument at a random time between 02:00 AM and 04:00 AM daily.

# Part 1 - Check if scheduled task exists and create if it doesn't
# PartVersion 1.0
#LOCK=OFF
# -----
$TaskName = "SO WHS SEND SALES"
...
if (-not $taskExists) {
    ...
    $Description = "Executes the SOScheduler WHSSENDSALES D argument for daily data transfer."
    $ActionExecute = "C:\Program Files (x86)\StationMaster\SOScheduler.exe"
    $ActionArgument = "WHSSENDSALES D"
